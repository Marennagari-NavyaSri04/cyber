...
class LSTMCNNFeatureExtractor:
    ...
    def _build_model(self):
        ...
        x = Conv1D(self.cnn_filters, self.kernel_size, activation="relu", padding="same")(inputs)
        x = MaxPooling1D(pool_size=2)(x)

        x = Conv1D(self.cnn_filters * 2, self.kernel_size, activation="relu", padding="same")(x)
        x = MaxPooling1D(pool_size=2)(x)

        x = LSTM(self.lstm_units, return_sequences=False)(x)

        x = Dense(self.feature_dim, activation="relu", name="deep_features")(x)
        x = Dropout(0.3)(x)

        model = Model(inputs, x)
        model.compile(
            optimizer=Adam(self.learning_rate),
            loss="mse"  # dummy loss for feature learning
        )
        return model

    def fit(self, X, y=None, epochs=2, batch_size=32, verbose=1):
        """
        Train extractor (unsupervised / weakly supervised)
        """
        if y is None:
            y = np.zeros((X.shape[0], self.feature_dim))
...
class LSTM_CNN_classifier:
    ...
    def fit(self, X, y):
        ...
        extractor = LSTMCNNFeatureExtractor(
